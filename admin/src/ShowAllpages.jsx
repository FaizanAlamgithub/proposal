...
function ShowAllpages() {
  ...
  const downloadPDF = async () => {
    ...
    let quality = 1; // Start with decent quality
    let scale = 3; // Balanced scale for clarity & compression
    let pdf;
    let fileSizeKB = 0;
    ...
    const fetchProposal = async () => {
      try {
        const response = await fetch(
          `https://proposal-backend-r6dj.onrender.com/api/proposals/${id}`
        );
        if (!response.ok) {
  ...
  return (
    <>
      ...
      <button
        onClick={downloadPDF}
        disabled={downloading}
        className="download-btn absolute left-0 bottom-[10%] z-10 flex items-center gap-2 px-4 py-2 bg-blue-600 text-white font-semibold hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed"
      >
        {downloading ? (
          <>
